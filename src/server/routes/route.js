...
let oAuth2Client = ''
//let entradaArchivo=document.getElementById('archivo');
...
route.get('/code-google', (request, response) => {
  ...
  const archivo =  request._parsedOriginalUrl.query +"";

  const nombreCortado = archivo.split('&');

  const final = nombreCortado[0].split('=');

  const token= final[1];

  response.render('verify-google', { title: 'Verify Google' ,token })
